<template>
  <v-timeline density="" direction="vertical" truncate-line="both">
    <v-timeline-item dot-color="green" icon="mdi-whistle">
      <template v-slot:opposite>
        <v-card class="ml-3">
          <v-card-title :class="['text-h6', `bg-green`]"> 0:0 </v-card-title>
        </v-card>
      </template>
      <v-card class="ml-3">
        <v-card-title :class="['text-h6', `bg-green`]"> Kickoff </v-card-title>
      </v-card>
    </v-timeline-item>
    ...
    <v-timeline-item
      ...
      :key="i"
      :dot-color="event?.iconColor ? event?.iconColor : event?.color"
      :size="event?.size"
      :icon="event?.icon"
      fill-dot
    >
      <v-tooltip v-if="event?.tooltip" :text="event?.tooltip" location="bottom">
        <template v-slot:activator="{ props: tooltipProps }">
          <v-card
            v-bind="tooltipProps"
            v-if="event?.title"
            @click.stop="props.drilldown(event)"
            elevation="3"
            :color="event.color"
          >
            ...
            <v-card-text
              v-if="event?.text"
              class="py-2 text-body-2 bg-white"
              v-html="event?.text"
            ></v-card-text>
          </v-card>
        </template>
      ...
      <v-card
        v-if="event?.title && !event?.tooltip && event?.clickable"
        @click.stop="props.drilldown(event)"
        elevation="3"
        :color="event.color"
      >
    ...
    <v-timeline-item dot-color="green" icon="mdi-whistle">
      ...
      <v-card class="mr-3">
        <v-card-title :class="['text-h6', `bg-green`]"> Fulltime </v-card-title>
      </v-card>
    </v-timeline-item>
  </v-timeline>
</template>
      ...
      .map((event) => {
        ...
        if (event.touchdown) {
          ...
          return {
            color:
              event.team === "0"
                ? homeTeamColours.value.primary
                : awayTeamColours.value.primary,
            icon: "mdi-football",
            iconColor: "brown",
            size: "large",
            ...
            text: `${dataStore
              ...
              .replaceAll(" ", "&nbsp;")}<br>${dataStore
              .getPlayerName(event.touchdownScorer || "")
              .replaceAll(" ", "&nbsp;")}`,
            opp: `Turn: ${event.turn}`,
